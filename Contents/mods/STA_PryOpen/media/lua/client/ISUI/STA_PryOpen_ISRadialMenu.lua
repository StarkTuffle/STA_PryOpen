...
function ISVehicleMenu.showRadialMenuOutside(playerObj)
    ...
    if vehicle then
        ...
        local target = { type = "vehicle", vehicle = vehicle, part = doorPart, player = playerObj }
            ...
            if doorPart and doorPart:getDoor() and doorPart:getInventoryItem() then
                ...
                if doorPart:getId() ~= "EngineDoor" then
                    ...
                    elseif not Utils.meetsStrengthRequirement(playerObj, category) then
                        ...
                        local label = Utils.buildLabelForTarget({type="vehicle",part=doorPart})
                        menu:addSlice(label, getTexture("media/ui/vehicles/CrowbarYes.png"), ContextMenu.onPrySelect, target)
                    end
                end
            end
        end
